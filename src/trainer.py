...
class VAETrainer(pl.LightningModule):
    ...
    def training_step(self, batch):
        ...
        if target_batch is not None:
            recon_loss, kl_z, kl_l, classification_loss, probs = self.VAE(
                data_batch, mean, variance, batch_index, target_batch
            )
            loss = torch.mean(recon_loss + kl_z + kl_l)
            loss += classification_loss
        else:
            recon_loss, kl_z, kl_l, probs = self.VAE(
                data_batch, mean, variance, batch_index
            )
            loss = torch.mean(recon_loss + kl_z + kl_l)
        return loss
    ...
    def prediction_step(self, batch):
        data_batch, target_batch, batch_index, mean, variance = batch
        _, _, _, classification_loss, probs = self.VAE(
            data_batch, mean, variance, batch_index, target_batch
        )
